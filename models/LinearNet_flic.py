...
class LinearNet_flic(FunctionSet):

    """
    One layer neural networks with batch Normalization
    """

    def __init__(self, size=220, num_joints=7):
        """
        - size: Input image is size of size x size
        - num_joints: number of joints to predict
        """
        super(LinearNet_flic, self).__init__(
            fc=F.Linear(size * size * 3, num_joints * 2)  # the normalized picture size
